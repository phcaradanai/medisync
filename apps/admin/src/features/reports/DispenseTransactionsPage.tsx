...
export function DispenseTransactionsPage() {
  ...
  const [emergencyTransactions, setEmergencyTransactions] = useState<EmergencyDispenseTransaction[]>([]);
  const [transactionType, setTransactionType] = useState<"prescription" | "emergency">("prescription");
  ...
  const emergencyRequest = useCallback((pageToken = "") => create(ListEmergencyDispenseTransactionsRequestSchema, {
    kioskCode,
    hn: prescription.trim(),
    employeeCode: employeeCode.trim(),
    drugCode: drugCode.trim(),
    statuses: status ? [Number(status) as EmergencyDispenseStatus] : [],
    operatorAuthMethods: authMethod ? [Number(authMethod) as EmergencyOperatorAuthMethod] : [],
    createdFrom: dateBoundary(createdFrom, false),
    createdTo: dateBoundary(createdTo, true),
    pageSize: 200,
    pageToken,
  }), [authMethod, createdFrom, createdTo, drugCode, employeeCode, kioskCode, prescription, status]);

  const load = useCallback(async () => {
    setLoading(true); setError(null);
    try {
      if (transactionType === "emergency") {
        const response = await dispensingClient.listEmergencyDispenseTransactions(emergencyRequest());
  ...
  useEffect(() => {
    setStatus(""); setPrescription(""); setDrugCode(""); setEmployeeCode(""); setAuthMethod("");
  }, [transactionType]);
  ...
  const totals = useMemo(() => transactionType === "emergency" ? ({
    dispensed: emergencyTransactions.filter((tx) => tx.status === EmergencyDispenseStatus.DISPENSED).length,
    failed: emergencyTransactions.filter((tx) => tx.status === EmergencyDispenseStatus.FAILED).length,
    units: emergencyTransactions.reduce((sum, tx) => sum + tx.dispensedQuantity, 0),
  }) : ({
  ...
  const exportCSV = async () => {
    ...
    try {
      if (transactionType === "emergency") {
        ...
        let token = "";
        do {
          const response = await dispensingClient.listEmergencyDispenseTransactions(emergencyRequest(token));
          all.push(...response.transactions); token = response.nextPageToken;
        } while (token);
        const header = ["transaction_type", "dispense_id", "kiosk_code", "hn", "employee_code", "operator", "auth_method", "drug_code", "drug_name", "slot_code", "requested_qty", "dispensed_qty", "status", "reason", "failure_code", "failure_detail", "trace_id", "queued_at", "started_at", "completed_at", "failed_at"].map(csvCell).join(",");
        const rows = all.map((tx) => ["EMERGENCY", tx.dispenseId, tx.kioskCode, tx.hn, tx.employeeCode, tx.operatorDisplayName, tx.operatorAuthMethod === EmergencyOperatorAuthMethod.CARD ? "CARD" : "EMPLOYEE_CODE", tx.drugCode, tx.drugName, tx.slotCode, tx.requestedQuantity, tx.dispensedQuantity, EMERGENCY_STATUS_LABEL[tx.status] ?? tx.status, tx.reason, tx.failureCode, tx.failureDetail, tx.traceId, dateTime(tx.queuedAt), dateTime(tx.startedAt), dateTime(tx.completedAt), dateTime(tx.failedAt)].map(csvCell).join(","));
        const blob = new Blob(["\uFEFF", header, "\r\n", rows.join("\r\n")], { type: "text/csv;charset=utf-8" });
        const url = URL.createObjectURL(blob); const anchor = document.createElement("a");
        anchor.href = url; anchor.download = `emergency-dispense-transactions-${new Date().toISOString().slice(0, 10)}.csv`; anchor.click(); URL.revokeObjectURL(url);
        return;
      }
  ...
  return <>
    <MasterHeader icon={Icon.clock} title="Dispense Transactions" subtitle={`${transactionType === "emergency" ? "Emergency" : "Prescription"} transactions ที่ตรวจสอบย้อนหลังได้ ${total} รายการ`}>
    ...
    <div className="md-kpis">
      <div className="md-kpi"><div className="md-kpi-label">รายการที่แสดง</div><div className="md-kpi-value">{transactionType === "emergency" ? emergencyTransactions.length : transactions.length}</div></div>
    ...
    <div className="md-panel">
      <div className="md-toolbar">
        <Select value={transactionType} onChange={(value) => setTransactionType(value as "prescription" | "emergency")}><option value="prescription">ตาม Prescription</option><option value="emergency">เบิกฉุกเฉิน</option></Select>
        <SearchInput value={prescription} onChange={setPrescription} placeholder={transactionType === "emergency" ? "ค้นหา HN" : "ค้นหา Prescription ID"}/>
        <SearchInput value={drugCode} onChange={setDrugCode} placeholder="ค้นหารหัสยา"/>
        {transactionType === "emergency" && <SearchInput value={employeeCode} onChange={setEmployeeCode} placeholder="ค้นหารหัสพนักงาน"/>}
        ...
        <Select value={status} onChange={setStatus}><option value="">ทุกสถานะ</option>{transactionType === "emergency" ? [EmergencyDispenseStatus.QUEUED, EmergencyDispenseStatus.DISPENSING, EmergencyDispenseStatus.DISPENSED, EmergencyDispenseStatus.FAILED].map((value) => <option key={value} value={value}>{EMERGENCY_STATUS_LABEL[value]}</option>) : STATUS_OPTIONS.map((value) => <option key={value} value={value}>{STATUS_LABEL[value]}</option>)}</Select>
        {transactionType === "emergency" && <Select value={authMethod} onChange={setAuthMethod}><option value="">ทุกวิธียืนยัน</option><option value={EmergencyOperatorAuthMethod.CARD}>สแกนบัตร</option><option value={EmergencyOperatorAuthMethod.EMPLOYEE_CODE}>รหัสพนักงาน</option></Select>}
        <label className="md-date-filter"><span>ตั้งแต่</span><input type="date" value={createdFrom} max={createdTo || undefined} onChange={(event) => setCreatedFrom(event.target.value)} /></label>
        <label className="md-date-filter"><span>ถึง</span><input type="date" value={createdTo} min={createdFrom || undefined} onChange={(event) => setCreatedTo(event.target.value)} /></label>
      </div>
      {transactionType === "emergency" ? <div className="md-table-wrap"><table className="md-table"><thead><tr><th>เวลา</th><th>Dispense ID</th><th>HN</th><th>ตู้ / ช่อง</th><th>ผู้เบิก</th><th>ยา</th><th>จ่ายจริง</th><th>สถานะ</th><th>รายละเอียด</th></tr></thead><tbody>
        {loading ? <tr><td colSpan={9}><div className="md-empty">กำลังโหลด…</div></td></tr> : emergencyTransactions.length === 0 ? <tr><td colSpan={9}><div className="md-empty">ไม่พบ Emergency transaction ตามตัวกรอง</div></td></tr> : emergencyTransactions.map((tx) => <tr key={tx.dispenseId}>
          <td className="md-cell-muted">{dateTime(tx.createdAt)}</td><td><span className="md-code" title={tx.dispenseId}>{tx.dispenseId.slice(0, 8)}</span></td><td>{tx.hn}</td><td><span className="md-code">{tx.kioskCode} / {tx.slotCode}</span></td><td>{tx.operatorDisplayName || tx.employeeCode}<br/><small>{tx.operatorAuthMethod === EmergencyOperatorAuthMethod.CARD ? "บัตร" : "รหัสพนักงาน"}</small></td><td>{tx.drugCode} — {tx.drugName}</td><td><strong>{tx.dispensedQuantity}</strong> / {tx.requestedQuantity}</td><td><span className={`md-badge ${tx.status === EmergencyDispenseStatus.DISPENSED ? "md-badge-on" : tx.status === EmergencyDispenseStatus.FAILED ? "md-badge-error" : "md-badge-warn"}`}>{EMERGENCY_STATUS_LABEL[tx.status] ?? "ไม่ทราบ"}</span></td><td className="md-cell-muted" title={tx.failureDetail}>{tx.failureDetail || tx.reason || "—"}</td>
        </tr>)}</tbody></table></div> :
